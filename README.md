...
[vot-dist]: https://raw.githubusercontent.com/carielis/voice-over-translation/master/dist/vot.user.js
...
### Аккаунт в нативном расширении

Вход выполняется через окно авторизации; ручной ввод токена на странице сайта
доступен только в версии для Tampermonkey/Violentmonkey. Нативное расширение
хранит токен в фоне и передаёт его только API Яндекса или встроенным прокси.
Пользовательские прокси поддерживают перевод без авторизации, но не получают
токен аккаунта. [Подробнее о границах доступа](./src/extension/README.md).
