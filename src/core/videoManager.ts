...
type ResolveVideoLanguageInput = {
  videoId: string;
  isStream: boolean;
  possibleLanguage: unknown;
  subtitles?: unknown;
...
function preserveYoutubeVolumeStorage<T>(action: () => T): T {
  ...
  try {
    ...
    // Only restore if we successfully read localStorage.
    // `undefined` means the read failed – do nothing.
    if (snapshot !== undefined) {
      try {
        if (snapshot === null) {
          globalThis.localStorage.removeItem(YT_PLAYER_VOLUME_STORAGE_KEY);
        } else {
          globalThis.localStorage.setItem(
            YT_PLAYER_VOLUME_STORAGE_KEY,
            snapshot,
          );
        }
      } catch {
        // localStorage can be blocked in some frames.
      }
    }
  }
}
...
function resolveYoutubeDetectedLanguageFromSubtitles(
  ...
  const candidates = subtitles.filter(
    (subtitle): subtitle is YoutubeSubtitleCandidate =>
      Boolean(subtitle) &&
      typeof subtitle === "object" &&
      (subtitle as YoutubeSubtitleCandidate).source === "youtube" &&
      typeof (subtitle as YoutubeSubtitleCandidate).translatedFromLanguage !==
        "string",
  );

  const pickLanguage = (
    predicate: (candidate: YoutubeSubtitleCandidate) => boolean,
  ): RequestLang | undefined =>
    candidates
      .filter(predicate)
      .map((candidate) => normalizeToRequestLang(candidate.language))
      .find(isResolvedLanguage);

  return (
    pickLanguage((candidate) => candidate.isAutoGenerated !== true) ??
    pickLanguage(() => true)
  );
}
...
export class VOTVideoManager {
  ...
  private async resolveVideoLanguage({
    videoId,
    isStream,
    possibleLanguage,
    subtitles,
  ...
  }: ResolveVideoLanguageInput): Promise<
    ResolveDetectedLanguageResult & {
      sharedLanguageState: SharedLanguageState;
    }
  > {
    const sharedLanguageState = getSharedLanguageState(videoId);
    ...
    if (result.cacheLanguage) {
      this.setDetectedLanguageCache(videoId, result.cacheLanguage);
    }
  ...
  async getVideoData() {
    ...
    const { detectedLanguage, sharedLanguageState } =
      await this.resolveVideoLanguage({
        videoId,
        isStream,
        possibleLanguage,
        subtitles,
